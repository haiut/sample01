...
class Tests_trdemo_client(unittest.TestCase):
    ...
    def test_5_get_cart(self):
        # GET http://trdemo-client/cart (endp 5)
    ...
    def test_6_get_flight(self, data_row):
        flight_id, = data_row

        # GET http://trdemo-client/flight (endp 6)
        trdemo_client = get_http_target('TARGET_TRDEMO_CLIENT', authenticate)
    ...
    def test_7_get_search(self, data_row):
        startDate, = data_row

        # GET http://trdemo-client/search (endp 7)
        trdemo_client = get_http_target('TARGET_TRDEMO_CLIENT', authenticate)
...
class Tests_trdemo_flights(unittest.TestCase):
    ...
    def test_1_get_flight_product_id_param(self, data_row):
        param, product_id = data_row

        # GET http://trdemo-flights/flight/{product_id}/{param} (endp 1)
    ...
    def test_2_get_flight_product_id(self, data_row):
        product_id, = data_row

        # GET http://trdemo-flights/flight/{product_id} (endp 2)
        trdemo_flights = get_http_target('TARGET_TRDEMO_FLIGHTS', authenticate)
        resp = trdemo_flights.get(f'/flight/{product_id}')
        resp.assert_ok()
        # resp.assert_status_code(200)
        # resp.assert_jsonpath('$.source', expected_value='TLV')
...
class Tests_trdemo_shoppingcart(unittest.TestCase):
    ...
    def test_4_get_cart_param(self, data_row):
        param, = data_row

        # GET http://trdemo-shoppingcart/cart/{param} (endp 4)
...
class Tests_trdemo_users(unittest.TestCase):
    ...
    def test_3_get_user_param(self, data_row):
        param, = data_row

        # GET http://trdemo-users/user/{param} (endp 3)
